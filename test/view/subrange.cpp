...
int main()
{
	...
	CPP_assert(Same<subrange<int*>,
		decltype(::algorithm(std::declval<int(&)[42]>()))>);
	CPP_assert(Same<subrange<std::vector<int>::iterator>,
	...
	CPP_assert(Constructible<subrange<Base*, Base*>, Base*, Base*>);
	CPP_assert(!Constructible<subrange<Base*, Base*>, Derived*, Derived*>);
	CPP_assert(Constructible<subrange<const Base*, const Base*>, Base*, Base*>);
	...
	CPP_assert(!SizedRange<decltype(l1)>);
	...
	}
	{
		...
		CPP_assert(Same<decltype(l0), decltype(s3)>);
	}
	{
		subrange s0{r0};
		subrange s1{l0};
	...
	}
	{
		...
		CPP_assert(Same<decltype(r0), decltype(s0)>);
		CPP_assert(Same<decltype(l0), decltype(s1)>);
		CPP_assert(Same<decltype(l0), decltype(s2)>);
	}
#endif
