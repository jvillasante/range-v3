        ...
        namespace detail
        {
            ...
            (
                ...
                concept PairLikeGCCBugs2_,
                    requires(T t) (
                        concepts::implicitly_convertible_to<
                            meta::_t<std::tuple_element<0, T>> const &>( get<0>(t) ),
                        concepts::implicitly_convertible_to<
                            meta::_t<std::tuple_element<1, T>> const &>( get<1>(t) )
                    )
            );
            CPP_def
            (
                ...
                concept PairLikeGCCBugs3_,
                    ranges::defer::Type<meta::_t<std::tuple_element<0, meta::_t<std::remove_const<T>>>>,
                         meta::_t<std::tuple_element<1, meta::_t<std::remove_const<T>>>>> &&
            ...
            (
                ...
                concept PairLikeGCCBugs_,
                    ranges::defer::DerivedFrom<std::tuple_size<T>, std::integral_constant<std::size_t, 2>> &&
                    defer::PairLikeGCCBugs3_<T>
            );
            CPP_def
            (
                template(class T)
                concept PairLike_,
                    ranges::defer::True<not std::is_reference<T>::value> &&
            ...
            (
                ...
                concept PairLikeConvertibleFromGCCBugs_,
                    defer::ConvertibleToNotSlicing_<U, meta::_t<std::tuple_element<0, T>>> &&
                    ranges::defer::ConvertibleTo<V, meta::_t<std::tuple_element<1, T>>>
            );
            CPP_def
            (
                template(class T, class U, class V)
                concept PairLikeConvertibleFrom_,
                    not ranges::defer::Range<T> &&
                    defer::PairLike_<T> &&
                    ranges::defer::Constructible<T, U, V> &&
                    ranges::defer::Type<meta::_t<std::tuple_size<T>>> && // GCC bugs
